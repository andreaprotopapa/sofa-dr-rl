...
from TrunkWallToolbox import rewardShaper, goalSetter
...
def add_visuals_and_solvers(root, config, visu, simu, fricionCoeff):
    if visu:
        ...
        spotLoc = [2*source[0], 100, 6*source[2]]
        root.addObject("SpotLight", position=spotLoc, direction=[-np.sign(source[0]), 0.0, -np.sign(source[2])])
        root.addObject("InteractiveCamera", name='camera', position=source, lookAt=target, zFar=1000)
...
def add_box_point(root, name, pos, color, size=2):
    ...
    box_visu.addObject('MeshOBJLoader', name='loader', filename=path+"ball.obj", scale3d=[size, size, size], translation = pos)
...
def CreateObject(node, name, surfaceMeshFileName, visu, simu, translation=[0., 0., 0.], rotation=[0., 0., 0.],
                 uniformScale=[1., 1., 1.], totalMass=1.,
    ...
    if visu:
        ...
        objectVisu.addObject('OglModel', name="model", src="@loader", scale3d=uniformScale, color=color,
                             updateNormals=False)
...
def createScene(rootNode, config={"source": [-600.0, -25, 100],
    ...
    if config['test'] and config['wallTranslation_distr'] is not None:
        config['wallTranslation_distr'] = None
        uniform_training = True
    cube_trasl, cube_rot, cubeScale, trunkScale, wallTranslation, trunkMass, trunkPoissonRatio, trunkYoungModulus = utils_rand.set_initial_state_distr(config) # sample new random (or static) dynamics
    cube_mass, frictionCoeff = utils_rand.set_dynamic_params(config)
    ...
    CreateObject(rootNode, name="Floor", surfaceMeshFileName="mesh/floor.obj", visu=visu, simu=simu, color=[1, 0.5, 0.5, 0.5],
                  uniformScale=[4, 6, 4], rotation=[0, 0, 0], translation=[0.0, config["floorHeight"], 0.0], isAStaticObject=True)
    
    CreateObject(rootNode, name="Wall", surfaceMeshFileName="mesh/square1.obj", visu=visu, simu=simu, color=[1, 0.5, 0.5, 0.5],
                  uniformScale=[1200, 600, 600], rotation=[0, 90, 0], translation=wallTranslation, isAStaticObject=True)
    ...
    config['goalPos'][0] = wallTranslation[0]
    ...
    if uniform_training:
        pos_init = config["wallTranslation_min_search"]
        pos_init[2] = 200
        add_box_point(rootNode, "wall_0", pos_init, [0, 0, 1, 1], size=3)
        pos_init = config["wallTranslation_max_search"]
        pos_init[2] = 200
        add_box_point(rootNode, "wall_1", pos_init, [0, 0, 1, 1], size=3)
        pos_init = wallTranslation
        pos_init[2] = 200
        add_box_point(rootNode, "wall_init", pos_init, [1, 0, 1, 1])
