...
def do_animate(task, _getReward_, _getState_, _startCmd, _getPos):
    ...
    #print(f"GoalPos from config['goalPos'] in rpc_client {config['goalPos']}")
    ...
    position = pos
    obs = _getState(root)
    done, reward, info = _getReward(root) # info must be added to each env in Toolbox - getReward
    ...
    s.taskDone(stateId, history, make_result(stateId, obs, reward, done, info))
...
def make_result(state_id, obs, reward, done, info):
