...
def TRS_to_matrix(translation, rotation=None, scale=None, eulerRotation=None):
    ...
    if eulerRotation != None:
        rotation = Quat.createFromEuler( eulerRotation, inDegree=True)
...
class RigidDof(object):
    ...
    def getPosition(self, index=0, field="position"):  
        print("GET POSITION", type(self.rigidobject.getData(field).value), field, index)  
...
class Transform(object):
    def __init__(self, translation, orientation=None, eulerRotation=None):
        ...
        if eulerRotation is not None:
            self.orientation = Quat.createFromEuler( eulerRotation, inDegree=True )
    ...
    def toSofaRepr(self):
            return list(self.translation) + list(self.orientation)
