...
class Dropo(object):
	...
	def __init__(self,
		...
		if (self.sim_env.get_attr('config')[0]['scene'] == 'Trunk' or self.sim_env.get_attr('config')[0]['scene'] == 'TrunkCube') and self.clip_state is not None:
			self.random_tuple_id = random.sample(range(21), clip_state)
	...
	def optimize_dynamics_distribution_resetfree(self,
		...
		if not resume:
			...
			phi = np.load(resume_path+'/'+unmodeled+'best_phi.npy')
			search_space_resumed = []
			for i, (mean, stdev) in enumerate(zip(self.get_means(phi), self.get_stdevs(phi))):
				width = max_task[i]-min_task[i] # Search interval for this parameter
				# MEAN
				if normalize:	# Normalize parameter mean to interval [0, 4]
					search_space_resumed.append(ng.p.Scalar(init=mean).set_bounds(lower=0, upper=self.normalized_width))
				else:
					search_space_resumed.append(ng.p.Scalar(init=mean).set_bounds(lower=min_task[i], upper=max_task[i]))
				# STD
				initial_std = width/8	# This may sometimes lead to a stdev smaller than the lower threshold of 0.00001, so take the minimum
				stdev_lower_bound = np.min([0.00001, initial_std-1e-5])
				stdev_upper_bound = width/4
				if normalize: # Normalize parameter stdev to interval [0, 4]
					search_space_resumed.append(ng.p.Scalar(init=stdev).set_bounds(lower=0, upper=self.normalized_width))
				else:	# Optimize parameters in their original scale (not recommended when using CMA-ES with the identity matrix as starting cov_matrix)
					search_space_resumed.append(ng.p.Scalar(init=stdev).set_bounds(lower=stdev_lower_bound, upper=stdev_upper_bound))
		...
		# Run optimization problem
		start = time.time() if not resume else wandb.config.start_time
		if not resume:
			wandb.config.update({"start_time":start}, allow_val_change=True)
		time_per_iter = 0 if not resume else wandb.config.time_per_iter

		print('CMA population size:', optim.es.popsize)
		wandb.run.summary["CMA_pop_size"] = optim.es.popsize
		i_start = 0 if not resume else wandb.config.opt_iteration_i - 1
		tot_opt_iterations = math.ceil(budget / optim.es.popsize)
		temperature = episode_length if not resume else wandb.config.temperature

		for i in range(i_start, tot_opt_iterations):
			...
			wandb.config.update({"tot_opt_iterations":wandb.run.summary["opt_iteration_tot"]}, allow_val_change=True)
			wandb.run.summary["opt_iteration_i"] = i+1
			wandb.config.update({"opt_iteration_i":wandb.run.summary["opt_iteration_i"]}, allow_val_change=True)
			wandb.run.summary["est_time_left"] = (self._seconds_to_human_time(time_per_iter*(tot_opt_iterations-i)) if time_per_iter != 0 else "-")
			wandb.config.update({"est_time_left":wandb.run.summary["est_time_left"]}, allow_val_change=True)
			...
			wandb.run.summary["time_per_iter"] = time_per_iter
			wandb.config.update({"time_per_iter":wandb.run.summary["time_per_iter"]}, allow_val_change=True)

			recommendation = optim.recommend()
			phi = recommendation.value[1]['phi']
			score = self._L_target_given_phi_resetfree(**recommendation.kwargs)
			elapsed = time.time()-start
			if wandb is not None:
				self._plot_phi_on_wandb(wandb, phi)
				wandb.log({"temperature": temperature})
				wandb.config.update({"temperature":temperature}, allow_val_change=True)
			filename = 'RF-DROPO_output'+'.txt'
			with open(os.path.join(run_path, filename), 'w', encoding='utf-8') as file:
				self.output_results(phi, self.sim_env.get_task(), score, elapsed, temperature, file=file)
			np.save(run_path+unmodeled+"best_phi.npy", phi)
			with open(os.path.join(run_path,'wandb_config.json'), 'w') as file:
				json.dump(wandb.config.as_dict(), file)
	...
	def _L_target_given_phi_resetfree(self,
									  ...
									  normalize,
									  temperature=None):
		"""Objective function evaluation for --sparse-mode and for resetfree"""
		if normalize:
		...
		for t in transitions:
			...
			if self.clip_state is not None:
				"""self.clip_state is the number of (x,y,z) coordinates to sample"""

				# downsample s_prime_samples   # (sample_size, 63+3+7)
				# downsample real_s_prime      # (63+3+7)
				# trunk
				if self.sim_env.get_attr('config')[0]['scene'] == 'Trunk':
					s_prime_samples_trunk = s_prime_samples[:,:-3]
					s_prime_samples_goal = s_prime_samples[:,-3:]
					real_s_prime_trunk = real_s_prime[:-3]
					real_s_prime_goal = real_s_prime[-3:]
					assert s_prime_samples_trunk.shape[1]%3==0
					assert s_prime_samples_goal.shape[1] == 3
					tupled_samples = np.array([[(row[i], row[i+1], row[i+2]) for i in range(0, len(row)-2,3)] for row in s_prime_samples_trunk])
					tupled_prime = np.array([(real_s_prime_trunk[i], real_s_prime_trunk[i+1], real_s_prime_trunk[i+2]) for i in range(0, len(real_s_prime_trunk)-2,3)])

					subsampled_s_prime_samples = np.array([row_tuples[self.random_tuple_id] for row_tuples in tupled_samples])
					subsampled_s_prime_samples = subsampled_s_prime_samples.reshape(s_prime_samples_trunk.shape[0],self.clip_state*3)
					subsampled_real_s_prime = tupled_prime[self.random_tuple_id]
					subsampled_real_s_prime = subsampled_real_s_prime.reshape(self.clip_state*3)

					s_prime_samples = np.concatenate((subsampled_s_prime_samples,s_prime_samples_goal), axis=1)
					real_s_prime = np.concatenate((subsampled_real_s_prime, real_s_prime_goal))
				elif self.sim_env.get_attr('config')[0]['scene'] == 'TrunkCube':
					s_prime_samples_trunk = s_prime_samples[:,:-10]
					s_prime_samples_goal_cube = s_prime_samples[:,-10:]
					real_s_prime_trunk = real_s_prime[:-10]
					real_s_prime_goal_cube = real_s_prime[-10:]
					assert s_prime_samples_trunk.shape[1]%3==0
					assert s_prime_samples_goal_cube.shape[1] == 10
					...
					s_prime_samples = np.concatenate((subsampled_s_prime_samples,s_prime_samples_goal_cube), axis=1)
					real_s_prime = np.concatenate((subsampled_real_s_prime, real_s_prime_goal_cube))
	...
	def output_results(self, phi, gt, best_score, elapsed, temperature, file=None):
		print('\n-----------', file=file)
		print('RESULTS\n', file=file)

		best_phi = self._denormalize_bounds(phi)

		print('GROUND TRUTH dynamics parameters:', gt, '\n', file=file)

		print('Best means and st.devs:\n---------------', file=file)
		print(self.pretty_print_bounds(best_phi),'\n', file=file)
		...
		print('Temperature: ', temperature, file=file)
