...
class MoG:
    ...
    def gen(self, n_samples=1):
        ...
        mog_tsf = np.random.choice(list(range(len(self.xs))), p=self.a/sum(self.a))
        sample = self.xs[mog_tsf].gen()
        #import ipdb; ipdb.set_trace()
        #print(f"ii\n{ii}")
        #print(f"ns\n{ns}")
    ...
    def to_file(self, fname):
        with open(fname, "w") as f:
            f.write(f"mog\n")
            for weight, dist in zip(self.a, self.xs):
                f.write(f"{weight},")
                for v in dist.m:
                    f.write(f"{v},")
                for v in dist.P.flatten():
                    f.write(f"{v},")
                f.write(f"X,")
...
def mog_from_bounds_file(path):
    with open(path) as f:
        bounds, dists, current_dist = [], [], []
        ...
        for col in second_row:
            if len(col) == 0:
                ...
                current_dist.append(float(col))

    lens = np.array([len(x) for x in dists])

    # All Gaussians need to have the same number of dimensions
    assert np.std(lens) == 0.

    bounds = np.array(dists)

    # The number of dimensions
    # bounds.shape[1] is 1 + d + d*d, this is the lazy way
    d = int(np.sqrt((bounds.shape[1]-1)))

    # Make sure the dimensions are fine
    assert d*d + d + 1 == bounds.shape[1]
