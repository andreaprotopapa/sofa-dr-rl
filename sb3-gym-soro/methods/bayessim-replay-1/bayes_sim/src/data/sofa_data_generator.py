from bayes_sim.src.sim.pendulum import *
...
class SofaDataGenerator():
    def __init__(self, env=None, dataset=None,
                 episodes_per_params=1, seed=0,
                 params=None, 
        ...
        self.expert_observations = dataset['observations']
        self.expert_next_observations = dataset['next_observations']
        self.expert_actions = dataset['actions']
        self.expert_terminals = dataset['terminals']

        # Add noise to next observations ### <---- ASK
        ...
        if params is None:
            params = list(self.env.dynamics_indexes.values())
        self.params = params
    ...
    def rollout(self):
        ...
        ep_tuple = ({self.env.config['scene']:
                        {f"{i}": self.params[i] for i in range(len(cur_params))}},
                        {"observation": [], "action": []})
        ...
        for s, a, ns, d in zip(self.expert_observations,\
            ...
            if set_state:
                self.env.reset()
                if hasattr(self.env, 'set_sim_state'):
    ...
    def gen(self, n_samples, save=False, wandb=None):
        ...
        if wandb is not None:
            wandb.run.summary["collection_tot"] = n_samples
        for ep in tqdm(range(n_samples)):
            ...
            if wandb is not None:
                wandb.run.summary["collection_it"] = ep
        ...
        if save:
            filename = os.path.join(self.assets_path, "trunkcube_"+str(n_samples)+"_samples_"
