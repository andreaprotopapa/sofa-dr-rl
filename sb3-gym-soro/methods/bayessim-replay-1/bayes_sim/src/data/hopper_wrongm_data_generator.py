...
class HopperWrongMassDataGenerator():
    def __init__(self, demo_file="",
                 episodes_per_params=1, seed=1995,
                 params=[f"mass{i}" for i in range(4)],
        ...
        self.env = gym.make("WrongMassHopper-v0")
        ...
        self.expert_observations = np.load(glob.glob(os.path.join(demo_file, '*_observations.npy'))[0])
        self.expert_next_observations = np.load(glob.glob(os.path.join(demo_file, '*_nextobservations.npy'))[0])
        self.expert_actions = np.load(glob.glob(os.path.join(demo_file, '*_actions.npy'))[0])
        self.expert_terminals = np.load(glob.glob(os.path.join(demo_file, '*_terminals.npy'))[0])
