"""Training DR with a given distribution on Random_envs: 

  Example:

    [Training] python train_fixed_dr.py --env trunkcube-v0 --test_env trunkcube-v0 --seed 0 --now 1 -t 2000000 --run_path ./runs/fixed-DR --bounds_path ./BestBounds/TrunkCube/RFDROPO/bounds_A1S0X.npy --distribution_type truncnorm --wandb_mode disabled
...
def main():
    ...
    wandb.init(config=vars(args),
             ...
             dir = run_path,
             project="SoRo-RL",
             group=('DR_'+args.env+'_train' if args.group is None else args.group),
    ...
    print(pretty_print_bounds(env, bounds),'\n')
    ...
    if args.distribution_type == "truncnorm":
        ...
        env.set_dr_distribution(dr_type='truncnorm', distr=bounds)
